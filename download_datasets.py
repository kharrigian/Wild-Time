...
def download_arxiv(data_dir):
    download_gdrive(
        url='https://drive.google.com/uc?id=17JdUhLWMSm75Tw6z7JBegjz_eWNW4Bnz',
...
def download_drug(data_dir):
    download_gdrive(
        url='https://drive.google.com/drive/folders/1796kUMCTs8r0dnQjBiLt7YTm6ZtZAd3f',
        save_path=os.path.join(data_dir, 'Drug-BA'),
        is_folder=True
    )
    download_gdrive(
        url='https://drive.google.com/uc?id=179g3KTOG2mBTZzF6lxG6iodNp8xmdxpg',
...
def download_fmow(data_dir):
    download_gdrive(
        url='https://drive.google.com/uc?id=17Jplcs6QSCZb5pmE8xDKKwQZnrjBClsf',
...
def download_huffpost(data_dir):
    download_gdrive(
        url='https://drive.google.com/uc?id=17Jp9CiEG69zOjAnqCK0sHu8SeWQ76uPd',
...
def download_mimic(args, data_dir):
    if not os.path.exists('./raw-data/mimic_stay_dict.pkl'):
        get_stay_dict(data_dir)
    data = pickle.load(open('./raw-data/mimic_stay_dict.pkl', 'rb'))
...
def download_precipitation(data_dir):
    download_gdrive(
        url='https://drive.google.com/uc?id=19BnZT3VxYEtNIEj0fN76UcGG2GP2JP65',
...
def download_yearbook(data_dir):
    download_gdrive(
        url='https://drive.google.com/uc?id=17I50QFmMAowDRswOD0sr-uq9tQZdjM4v',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default='./raw-data/')
